...
public class MedidaBusquedaLineal {
    ...
    public static final int INCRTALLA = 100000, REPETICIONES = 1000;
    ...
    public static void medidaBusquedaLineal() {   
        ...
        for (int t = INITALLA; t <= MAXTALLA; t += INCRTALLA) {
            ...
            // OJO: Como es demasiado rápido, las repeticiones 
            // se incluyen en la medida de tiempo     
            ti = System.nanoTime();    // Tiempo inicial
            for (int r = 0; r < REPETICIONES; r++) {
                AlgoritmosMedibles.busquedaLineal(a, a[0]);
            }
            tf = System.nanoTime();   // Tiempo final
            double tMejor = (double) (tf - ti) / REPETICIONES; 
                                                  // Tiempo promedio 
                                                  // del caso mejor
      
            // Estudio del Caso promedio: buscar un número 
            // aleatorio entre 0 y t-1
            tt = 0;                        // Tiempo acumulado inicial a 0
            for (int r = 0; r < REPETICIONES; r++) {
                int aux = (int) Math.floor(Math.random() * t); // Núm a buscar
                ti = System.nanoTime();      // Tiempo inicial
            ...
            // Imprimir resultados
            System.out.printf(Locale.US, "%8d   %8.2f    %4.2f   %8.2f\n", 
                              t, tPeor / NMS, tMejor / NMS, tPromed / NMS);
        }
    }

    public static void main(String[] args) {
        medidaBusquedaLineal();
    }
}
